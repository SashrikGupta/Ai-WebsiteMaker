...
const run = async(text , front)=>{
   try {
      ...
      code = await fetch("http://localhost:2008/get_ml_api" , {
         method : "POST",
         headers : {
            "Content-Type" : "application/json"
         },
      ...
      note for frontend : you have to use api as demonstrated below 
                           ${code.result}
                           in the fontend according to query 
...
def start_servers():
    ...
    os.chdir("..")
    os.chdir("Js_server")
    actual_backend = subprocess.Popen(["nodemon" , "start"], shell=True)
    os.chdir("..")
...
@app.route('/get_ml_api', methods=['POST'])
def get_query_api():
    print("got hit")
...
if __name__ == '__main__':
    app.run(debug=True, use_reloader=False , port=2008)
